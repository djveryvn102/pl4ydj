	...
	var methods		= {
		destroy: function(){
			this.each(function(i, el){
				...
				self.off(mEvt.down);
				self.children('.xplayer-btn').off(mEvt.up).off(mEvt.down).remove();
				self.children('.xplayer-barload').off(mEvt.down).remove();
		...
		startAt: function(pct, animated){
			this.each(function(i, el){
				var self		= $(el);
				var knob		= self.children('.xplayer-btn');
				var follow		= self.children('.xplayer-barload');
				var guid		= self.attr('id');
				...
				// gui
				if (animated){
					knob.animate({'left': pxAdjust+'px'}, 0);
					follow.animate({'width': px+'px'}, 0);
				} else {
